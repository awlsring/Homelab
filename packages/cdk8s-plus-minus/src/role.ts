// yoinked from cdk8s-plus https://github.com/cdk8s-team/cdk8s-plus/blob/k8s-29/main/src/role.ts
...
export class ClusterRole extends base.Resource implements IClusterRole, IRole {
   ...
   * Imports a role from the cluster as a reference.
   */
  public static fromClusterRoleName(
    scope: Construct,
    id: string,
    name: string,
  ): IClusterRole {
  ...
  constructor(scope: Construct, id: string, props: ClusterRoleProps = {}) {
    ...
    this.apiObject = new k8s.KubeClusterRole(this, "Resource", {
      metadata: props.metadata,
      rules: Lazy.any({ produce: () => this.synthesizeRules() }),
      aggregationRule: Lazy.any({
        produce: () => this.synthesizeAggregationRules(),
      }),
    });
   ...
   * @param subjects a list of subjects to bind to
   */
  public bindInNamespace(
    namespace: string,
    ...subjects: rb.ISubject[]
  ): rb.RoleBinding {
    ...
    for (const rule of this._rules) {
      for (const endpoint of rule.endpoints) {
        ...
        if (resource && nonResource) {
          throw new Error(
            "Endpoint must be either resource or non resource. not both.",
          );
        }

        if (!resource && !nonResource) {
          throw new Error(
            "Endpoint must be either resource or non resource. not neither.",
          );
        }
