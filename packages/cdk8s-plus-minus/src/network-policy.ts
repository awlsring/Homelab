...
export class NetworkPolicyPort {
  ...
  public static tcp(port: number): NetworkPolicyPort {
    return new NetworkPolicyPort(
      k8s.IntOrString.fromNumber(port),
      undefined,
      NetworkProtocol.TCP,
    );
  }
  ...
  public static tcpRange(startPort: number, endPort: number) {
    return new NetworkPolicyPort(
      k8s.IntOrString.fromNumber(startPort),
      endPort,
      NetworkProtocol.TCP,
    );
  }
  ...
  public static of(props: NetworkPolicyPortProps): NetworkPolicyPort {
    return new NetworkPolicyPort(
      props.port ? k8s.IntOrString.fromNumber(props.port) : undefined,
      props.endPort,
      props.protocol,
    );
  }
 ...
 */
export class NetworkPolicyIpBlock
  extends Construct
  implements INetworkPolicyPeer
{
  ...
  public static ipv6(
    ...
    if (!cidrMatch[3]) {
      throw new Error(
        `CIDR mask is missing in IPv6: "${cidrIp}". Did you mean "${cidrIp}/128"?`,
      );
    }
...
export class NetworkPolicy extends base.Resource {
  ...
  public constructor(
    ...
    if (!props.metadata?.namespace) {
      if (
        this._podSelectorConfig.namespaces?.labelSelector &&
        !this._podSelectorConfig.namespaces?.labelSelector.isEmpty()
      ) {
        throw new Error(
          `Unable to create a network policy for a selector (${podSelector.node.path}) that selects pods in namespaces based on labels`,
        );
      }

      if (
        this._podSelectorConfig.namespaces?.names &&
        this._podSelectorConfig.namespaces.names.length > 1
      ) {
        throw new Error(
          `Unable to create a network policy for a selector (${podSelector.node.path}) that selects pods in multiple namespaces`,
        );
      }

      ns = this._podSelectorConfig.namespaces?.names
        ? this._podSelectorConfig.namespaces?.names[0]
        : undefined;
    } else {
  ...
  public addEgressRule(peer: INetworkPolicyPeer, ports?: NetworkPolicyPort[]) {
    ...
    this._egressRules.push({
      ports: (ports ?? []).map((p) => p._toKube()),
      to: this.createNetworkPolicyPeers(peer),
    });
  }
  ...
  public addIngressRule(peer: INetworkPolicyPeer, ports?: NetworkPolicyPort[]) {
    ...
    this._ingressRules.push({
      ports: (ports ?? []).map((p) => p._toKube()),
      from: this.createNetworkPolicyPeers(peer),
    });
  }
  ...
  private createNetworkPolicyPeers(
    ...
    if (!config.podSelector!.namespaces?.names) {
      ...
      return [
        {
          namespaceSelector:
            config.podSelector!.namespaces?.labelSelector?._toKube(),
  ...
  private configureDefaultBehavior(
    ...
    _default?: NetworkPolicyTrafficDefault,
  ) {
    if (!_default) {
      return;
    }
...
export function validatePeerConfig(peerConfig: NetworkPolicyPeerConfig) {
  if (!peerConfig.ipBlock && !peerConfig.podSelector) {
    throw new Error(
      "Inavlid peer: either 'ipBlock' or 'podSelector' must be defined",
    );
  }
  if (peerConfig.ipBlock && peerConfig.podSelector) {
    throw new Error(
      "Inavlid peer: only one of 'ipBlock' and 'podSelector' must be defined",
    );
  }
}
