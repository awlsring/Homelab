// yoinked from cdk8s-plus https://github.com/cdk8s-team/cdk8s-plus/blob/k8s-29/main/src/pod.ts
...
export abstract class AbstractPod
  extends base.Resource
  implements IPodSelector, networkpolicy.INetworkPolicyPeer, rb.ISubject
{
  ...
  constructor(scope: Construct, id: string, props: AbstractPodProps = {}) {
    ...
    this.terminationGracePeriod =
      props.terminationGracePeriod ?? Duration.seconds(30);
  ...
  public toPodSelectorConfig(): PodSelectorConfig {
    ...
    if (!podAddress) {
      ...
      throw new Error(
        `Unable to create a label selector since ${Pod.ADDRESS_LABEL} label is missing`,
      );
    }
    return {
      labelSelector: LabelSelector.of({
        labels: { [Pod.ADDRESS_LABEL]: podAddress },
      }),
  ...
  public toSubjectConfiguration(): rb.SubjectConfiguration {
    if (!this.serviceAccount && !this.automountServiceAccountToken) {
      throw new Error(
        `${this.name} cannot be converted to a role binding subject:` +
  ...
  public _toPodSpec(): k8s.PodSpec {
    ...
    function addVolume(vol: volume.Volume) {
      ...
      if (existingVolume && existingVolume !== vol) {
        throw new Error(
          `Invalid mount configuration. At least two different volumes have the same name: ${vol.name}`,
        );
      }
    ...
    return {
      ...
      volumes: undefinedIfEmpty(
        Array.from(volumes.values()).map((v) => v._toKube()),
      ),
      ...
      imagePullSecrets: this.dockerRegistryAuth
        ? [{ name: this.dockerRegistryAuth.name }]
        : undefined,
...
export class LabelSelector {
  ...
  public isEmpty() {
    return (
      this.expressions.length === 0 && Object.keys(this.labels).length === 0
    );
  }
  ...
  public _toKube(): k8s.LabelSelector {
    ...
    return {
      matchExpressions: undefinedIfEmpty(
        this.expressions.map((q) => ({
          key: q.key,
          operator: q.operator,
          values: q.values,
        })),
...
export class PodDns {
  ...
  public _toKube(): {
    ...
    if (this.policy === DnsPolicy.NONE && this.nameservers.length === 0) {
      throw new Error(
        "When dns policy is set to NONE, at least one nameserver is required",
      );
    }
...
export class PodSecurityContext {
  ...
  constructor(props: PodSecurityContextProps = {}) {
    ...
    this.fsGroupChangePolicy =
      props.fsGroupChangePolicy ?? FsGroupChangePolicy.ALWAYS;
...
export class NodeTaintQuery {
  ...
  public static is(
    key: string,
    value: string,
    options: NodeTaintQueryOptions = {},
  ): NodeTaintQuery {
    return new NodeTaintQuery(
      "Equal",
      key,
      value,
      options.effect,
...
export class PodScheduling {
  ...
  public assign(node: NamedNode) {
    if (this._nodeName) {
      ...
      throw new Error(
        `Cannot assign ${this.instance.podMetadata.name} to node ${node.name}. It is already assigned to node ${this._nodeName}`,
      );
    } else {
  ...
  public attract(node: LabeledNode, options: PodSchedulingAttractOptions = {}) {
    ...
    if (options.weight) {
      ...
      this._nodeAffinityPreferred.push({
  ...
  public colocate(
    ...
    options: PodSchedulingColocateOptions = {},
  ) {
    ...
    if (options.weight) {
      this.validateWeight(options.weight);
      this._podAffinityPreferred.push({
        weight: options.weight,
  ...
  public separate(
    selector: IPodSelector,
    options: PodSchedulingSeparateOptions = {},
  ) {
    const topology = options.topology ?? Topology.HOSTNAME;
    const term = this.createPodAffinityTerm(topology, selector);
    ...
    if (options.weight) {
      this.validateWeight(options.weight);
      this._podAntiAffinityPreferred.push({
        weight: options.weight,
        podAffinityTerm: term,
      });
    } else {
  ...
  private validateWeight(weight: number) {
    if (weight < 1 || weight > 100) {
      ...
      throw new Error(
        `Invalid affinity weight: ${weight}. Must be in range 1-100`,
      );
    }
  }
      ...
      affinity: hasAffinity
        ? {
            nodeAffinity: hasNodeAffinity
              ? {
                  preferredDuringSchedulingIgnoredDuringExecution:
                    undefinedIfEmpty(this._nodeAffinityPreferred),
                  requiredDuringSchedulingIgnoredDuringExecution:
                    this._nodeAffinityRequired.length > 0
                      ? {
            ...
            podAffinity: hasPodAffinity
              ? {
                  preferredDuringSchedulingIgnoredDuringExecution:
                    undefinedIfEmpty(this._podAffinityPreferred),
                  requiredDuringSchedulingIgnoredDuringExecution:
                    undefinedIfEmpty(this._podAffinityRequired),
                }
              : undefined,
            podAntiAffinity: hasPodAntiAffinty
              ? {
                  preferredDuringSchedulingIgnoredDuringExecution:
                    undefinedIfEmpty(this._podAntiAffinityPreferred),
                  requiredDuringSchedulingIgnoredDuringExecution:
                    undefinedIfEmpty(this._podAntiAffinityRequired),
                }
              : undefined,
          }
        : undefined,
...
export class PodConnections {
   ...
   */
  public allowTo(
    peer: networkpolicy.INetworkPolicyPeer,
    options: PodConnectionsAllowToOptions = {},
  ) {
    return this.allow("Egress", peer, {
      ports: this.extractPorts(peer),
      ...options,
    });
  }
  ...
  public allowFrom(
    peer: networkpolicy.INetworkPolicyPeer,
    options: PodConnectionsAllowFromOptions = {},
  ) {
    return this.allow("Ingress", peer, {
      ports: this.extractPorts(this.instance),
      ...options,
    });
  }
  ...
  private allow(
    ...
    peer: networkpolicy.INetworkPolicyPeer,
    options: PodConnectionsAllowToOptions | PodConnectionsAllowFromOptions = {},
  ) {
    ...
    if (
      !options.isolation ||
      options.isolation === PodConnectionsIsolation.POD
    ) {
      const src = new networkpolicy.NetworkPolicy(
        this.instance,
        `Allow${direction}${peerAddress}`,
        {
    ...
    if (
      !options.isolation ||
      options.isolation === PodConnectionsIsolation.PEER
    ) {
      if (config.ipBlock) {
      ...
      if (!podSelector) {
        throw new Error(
          `Unable to create policies for peer '${peer.node.addr}' since its not a pod selector`,
        );
      }
      ...
      if (!podSelectorConfig.namespaces) {
        ...
        if (
          podSelectorConfig.namespaces.labelSelector &&
          !podSelectorConfig.namespaces.labelSelector.isEmpty()
        ) {
          throw new Error(
            `Unable to create an ${oppositeDirection} policy for peer '${peer.node.path}' (pod=${this.instance.name}). Peer must specify namespaces only by name`,
          );
        }
        ...
        if (!podSelectorConfig.namespaces.names) {
          throw new Error(
            `Unable to create an ${oppositeDirection} policy for peer '${peer.node.path}' (pod=${this.instance.name}). Peer must specify namespace names`,
          );
        }
      ...
      for (const name of namespaces) {
        switch (direction) {
          case "Egress":
            new networkpolicy.NetworkPolicy(
              this.instance,
              `AllowIngress${name}${peerAddress}`,
              {
                selector: podSelector,
                metadata: { namespace: name },
                ingress: {
                  rules: [{ peer: this.instance, ports: options.ports }],
                },
              },
            );
            break;
  ...
  private extractPorts(
    selector?: networkpolicy.INetworkPolicyPeer,
  ): networkpolicy.NetworkPolicyPort[] {
    return container
      .extractContainerPorts(selector)
      .map((n) => networkpolicy.NetworkPolicyPort.tcp(n.number));
  }
