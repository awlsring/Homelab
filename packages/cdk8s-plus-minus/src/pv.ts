...
export class PersistentVolume
  ...
  implements IPersistentVolume, volume.IStorage
{
  /**
   * Imports a pv from the cluster as a reference.
   */
  public static fromPersistentVolumeName(
    scope: Construct,
    id: string,
    volumeName: string,
  ): IPersistentVolume {
  ...
  protected constructor(
    ...
    this.reclaimPolicy =
      props.reclaimPolicy ?? PersistentVolumeReclaimPolicy.RETAIN;
  ...
  public bind(claim: pvc.IPersistentVolumeClaim) {
    if (this._claim && this._claim.name !== claim.name) {
      throw new Error(
        `Cannot bind volume '${this.name}' to claim '${claim.name}' since it is already bound to claim '${this._claim.name}'`,
      );
    }
    this._claim = claim;
  }
  ...
  public _toKube(): k8s.PersistentVolumeSpec {
    const storage = this.storage
      ? k8s.Quantity.fromString(this.storage.toGibibytes() + "Gi")
      : undefined;
 ...
 */
export interface AwsElasticBlockStorePersistentVolumeProps
  extends PersistentVolumeProps {
  /**
...
export class AzureDiskPersistentVolume extends PersistentVolume {
  ...
  constructor(
    ...
    this.cachingMode =
      props.cachingMode ?? volume.AzureDiskPersistentVolumeCachingMode.NONE;
    this.fsType = props.fsType ?? "ext4";
 ...
 */
export interface GCEPersistentDiskPersistentVolumeProps
  extends PersistentVolumeProps {
  /**
