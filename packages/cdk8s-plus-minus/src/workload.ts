// yoinked from cdk8s-plus https://github.com/cdk8s-team/cdk8s-plus/blob/k8s-29/main/src/workload.ts
...
export abstract class Workload extends pod.AbstractPod {
  ...
  constructor(scope: Construct, id: string, props: WorkloadProps) {
    ...
    if (props.select ?? true) {
      this.select(
        pod.LabelSelector.of({
          labels: { [pod.Pod.ADDRESS_LABEL]: this._matcher },
        }),
      );
    }
  }
...
export class WorkloadScheduling extends pod.PodScheduling {
  ...
  public spread(options: WorkloadSchedulingSpreadOptions = {}) {
    this.separate(this.instance, {
      weight: options.weight,
      topology: options.topology ?? pod.Topology.HOSTNAME,
    });
  }
}
