// yoinked from cdk8s-plus https://github.com/cdk8s-team/cdk8s-plus/blob/k8s-29/main/src/deployment.ts
 ...
 */
export interface ExposeDeploymentViaIngressOptions
  extends DeploymentExposeViaServiceOptions,
    service.ExposeServiceViaIngressOptions {}
...
export class Deployment
  extends workload.Workload
  implements IScalable, IDeployment
{
  /**
   * Number of desired pods.
   */
  ...
  constructor(scope: Construct, id: string, props: DeploymentProps = {}) {
    ...
    if (this.progressDeadline.toSeconds() <= this.minReady.toSeconds()) {
      throw new Error(
        `'progressDeadline' (${this.progressDeadline.toSeconds()}s) must be greater than 'minReady' (${this.minReady.toSeconds()}s)`,
      );
    }
  ...
  public exposeViaService(
    ...
    const ports: service.ServicePort[] =
      options.ports ??
      myPorts.map((p) => ({
        port: p.number,
        targetPort: p.number,
    ...
    if (ports.length === 0) {
      throw new Error(
        `Unable to expose deployment ${this.name} via a service: ` +
    ...
    for (const port of ports) {
      ...
      if (!myPortNumbers.includes(targetPort)) {
        throw new Error(
          `Unable to expose deployment ${this.name} via a service: Port ${targetPort} is not exposed by any container`,
        );
      }
    }
...
export class PercentOrAbsolute {
  ...
  public isZero(): boolean {
    return (
      this.value === PercentOrAbsolute.absolute(0).value ||
      this.value === PercentOrAbsolute.percent(0).value
    );
  }
}
...
export class DeploymentStrategy {
  ...
  public static rollingUpdate(
    ...
    const maxUnavailable =
      options.maxUnavailable ?? PercentOrAbsolute.percent(25);
