...
export class Volume extends Construct implements IStorage {
   ...
   */
  public static fromAwsElasticBlockStore(
    scope: Construct,
    id: string,
    volumeId: string,
  ...
  public static fromAzureDisk(
    ...
    return new Volume(scope, id, options.name ?? `azuredisk-${diskName}`, {
      azureDisk: {
        ...
        diskUri,
        cachingMode:
          options.cachingMode ?? AzureDiskPersistentVolumeCachingMode.NONE,
        fsType: options.fsType ?? "ext4",
   ...
   */
  public static fromGcePersistentDisk(
    scope: Construct,
    id: string,
    pdName: string,
   ...
   */
  public static fromConfigMap(
    scope: Construct,
    id: string,
    configMap: configmap.IConfigMap,
    options: ConfigMapVolumeOptions = {},
  ): Volume {
    return new Volume(
      scope,
      id,
      options.name ?? `configmap-${configMap.name}`,
      {
        configMap: {
          name: configMap.name,
  ...
  public static fromPersistentVolumeClaim(
    ...
    options: PersistentVolumeClaimVolumeOptions = {},
  ): Volume {
   ...
   */
  public static fromNfs(
    scope: Construct,
    id: string,
    name: string,
    options: NfsVolumeOptions,
  ): Volume {
    return new Volume(scope, id, name, {
      nfs: {
  ...
  public static fromCsi(
    ...
    return new Volume(
      scope,
      id,
      options.name ?? Names.toDnsLabel(scope, { extra: [id] }),
      {
        csi: {
          driver: driver,
   ...
   * @internal
   */
  private static renderItems = (items?: {
    [key: string]: PathMapping;
  }): undefined | Array<k8s.KeyToPath> => {
    if (!items) {
      return undefined;
    }
