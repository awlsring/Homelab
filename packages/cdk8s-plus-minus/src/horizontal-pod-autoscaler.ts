...
export class HorizontalPodAutoscaler extends Resource {
  ...
  constructor(
    ...
    if (props?.minReplicas && props.minReplicas > props.maxReplicas) {
      throw new Error(
        `'minReplicas' (${props.minReplicas}) must be less than or equal to 'maxReplicas' (${props.maxReplicas}) in order for HorizontalPodAutoscaler to scale.`,
      );
    }
    if (props?.scaleUp?.stabilizationWindow !== undefined) {
      this._validateStabilizationWindow(
        "scaleUp",
        props.scaleUp.stabilizationWindow,
      );
    }
    if (props?.scaleDown?.stabilizationWindow !== undefined) {
      this._validateStabilizationWindow(
        "scaleDown",
        props.scaleDown.stabilizationWindow,
      );
    }
    ...
    this.scaleUp = {
      ...
      ...props.scaleUp,
      policies: props.scaleUp?.policies?.map((p) => ({
        duration: this._defaultScalingDuration,
        ...p,
      })) ?? [
        {
    ...
    this.scaleDown = {
      ...
      ...props.scaleDown,
      policies: props.scaleDown?.policies?.map((p) => ({
        duration: this._defaultScalingDuration,
        ...p,
      })) ?? [
        {
    ...
    this.node.addValidation({
      validate: () => this._validateTargetContainers(),
    });
  }
  ...
  private _validateScalingPolicyDuration(
    ...
    if (!isWithinRange) {
      throw new Error(
        `'${direction}.policies' duration (${duration.toHumanString()}) is outside of the allowed range. Must be at least 1 second long and no longer than 30 minutes.`,
      );
    }
  }
  ...
  private _validateStabilizationWindow(
    ...
    if (!isWithinRange) {
      throw new Error(
        `'${direction}.stabilizationWindow' (${window.toHumanString()}) must be 0 seconds or more with a max of 1 hour.`,
      );
    }
  }
  ...
  private _validateTargetContainers() {
    ...
    const hasResourceConstraints = containers.some((c) =>
      this._hasRequestsOrLimits(c),
    );
    if (!hasResourceConstraints && !this.metrics) {
      return [
        "If HorizontalPodAutoscaler does not have metrics defined, then every container in the target must have a CPU or memory resource constraint defined.",
      ];
    }
    return [];
  }
  ...
  private _hasRequestsOrLimits(c: container.Container): Boolean {
    const hasRequests =
      c.resources?.cpu?.request || c.resources?.memory?.request;
      ...
      behavior: {
        scaleUp: {
          policies: this.scaleUp.policies?.map((p) => ({
            ...p.replicas._toKube(),
            periodSeconds:
              p.duration?.toSeconds() ??
              this._defaultScalingDuration.toSeconds(),
          })),
          selectPolicy: this.scaleUp.strategy,
          stabilizationWindowSeconds:
            this.scaleUp.stabilizationWindow?.toSeconds(),
        },
        scaleDown: {
          ...
          stabilizationWindowSeconds:
            this.scaleDown.stabilizationWindow?.toSeconds(),
        },
      },
    };
  }
...
export class Metric {
   ...
   *
   */
  public static containerMemory(
    options: MetricContainerResourceOptions,
  ): Metric {
    return new Metric({
      type: "ContainerResource",
      containerResource: {
   ...
   *
   */
  public static containerStorage(
    options: MetricContainerResourceOptions,
  ): Metric {
    return new Metric({
      type: "ContainerResource",
   ...
   *
   */
  public static containerEphemeralStorage(
    options: MetricContainerResourceOptions,
  ): Metric {
    return new Metric({
      type: "ContainerResource",
...
export class Replicas {
  ...
  private constructor(
    private readonly replicas: Pick<k8s.HpaScalingPolicyV2, "type" | "value">,
  ) {}
