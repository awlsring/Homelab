...
export class CronJob extends workload.Workload {
  ...
  constructor(scope: Construct, id: string, props: CronJobProps) {
    ...
    if (
      props.startingDeadline != undefined &&
      props.startingDeadline.toSeconds() < 10
    ) {
      throw new Error(
        `The 'startingDeadline' property cannot be less than 10 seconds since the Kubernetes CronJobController checks things every 10 seconds and hence the CronJob may not be scheduled. The value passed is: ${props.startingDeadline}`,
      );
    }

    if (
      props.ttlAfterFinished != undefined &&
      (props.successfulJobsRetained != undefined ||
        props.failedJobsRetained != undefined)
    ) {
      throw new Error(
        "The 'ttlAfterFinished' property cannot be set if 'successfulJobsRetained' property or 'failedJobsRetained' property is set. This would cause the retention of jobs to not work properly since it would delete the job based on its value.",
      );
    }
    ...
    this.concurrencyPolicy =
      props.concurrencyPolicy ?? ConcurrencyPolicy.FORBID;
