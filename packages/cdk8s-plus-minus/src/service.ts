...
export interface IService extends IResource {
  ...
  exposeViaIngress(
    path: string,
    options?: ExposeServiceViaIngressOptions,
  ): ingress.IIngress;
}
...
export interface ExposeServiceViaIngressOptions {
   ...
   */
  readonly ingress?: ingress.IIngress;
}
...
export class Service extends base.Resource implements IService {
  ...
  public exposeViaIngress(
    ...
    ingr.addRule(
      path,
      ingress.IngressBackend.fromService(this),
      options.pathType,
    );
    return ingr;
  }
  ...
  public select(selector: pod.IPodSelector) {
    const labels =
      selector.toPodSelectorConfig().labelSelector._toKube().matchLabels ?? {};
    ...
    for (const port of this._ports) {
      ports.push({
        ...
        targetPort: port.targetPort
          ? k8s.IntOrString.fromNumber(port.targetPort)
          : undefined,
