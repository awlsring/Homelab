...
export interface IIngress extends IResource {
  addDefaultBackend(backend: IIngressBackend): void;
  addHostDefaultBackend(host: string, backend: IIngressBackend): void;
  addHostRule(
    host: string,
    path: string,
...
export interface IngressProps extends base.ResourceProps {
   ...
   */
  readonly defaultBackend?: IIngressBackend;
 ...
 */
export class Ingress extends base.Resource implements IIngress {
  /**
  ...
  private readonly _rulesPerHost: { [host: string]: k8s.HttpIngressPath[] } =
    {};
  private _defaultBackend?: IIngressBackend;
  ...
  constructor(scope: Construct, id: string, props: IngressProps = {}) {
    ...
    this.apiObject = new k8s.KubeIngress(this, "Resource", {
      ...
      spec: {
        defaultBackend: Lazy.any({
          produce: () => this._defaultBackend?._toKube(),
        }),
   ...
   */
  public addDefaultBackend(backend: IIngressBackend) {
   ...
   */
  public addHostDefaultBackend(host: string, backend: IIngressBackend) {
    if (!host) {
    ...
    for (const rule of rules) {
      ...
      if (!rule.host && !rule.path) {
        if (this._defaultBackend) {
          throw new Error(
            "a default backend is already defined for this ingress",
          );
        }
      ...
      const routes = (this._rulesPerHost[host] =
        this._rulesPerHost[host] ?? []);
...
export class IngressBackend {
  ...
  public static fromService(
    ...
    if (serv.ports.length === 1) {
      ...
      if (options.port !== undefined) {
        ...
        if (found) {
          ...
          throw new Error(
            `service exposes ports ${serv.ports
              .map((p) => p.port)
              .join(",")} but backend is defined to use port ${options.port}`,
          );
        }
      } else {
        throw new Error(
          `unable to determine service port since service exposes multiple ports: ${serv.ports
            .map((x) => x.port)
            .join(",")}`,
        );
      }
    ...
    if (options.port !== undefined && servicePort !== options.port) {
      throw new Error(
        `backend defines port ${options.port} but service exposes port ${servicePort}`,
      );
    }
   ...
   */
  public static fromResource(resource: base.IResource): IIngressBackend {
    return new IngressBackend({
      resource: {
        kind: resource.kind,
        name: resource.name,
...
export interface IngressRule {
   ...
   * be forwarded to.
   */
  readonly backend: IIngressBackend;
