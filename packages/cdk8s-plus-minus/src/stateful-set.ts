...
export class StatefulSet extends workload.Workload implements IScalable {
  ...
  constructor(scope: Construct, id: string, props: StatefulSetProps) {
    ...
    (this.strategy =
      props.strategy ?? StatefulSetUpdateStrategy.rollingUpdate()),
      (this.podManagementPolicy =
        props.podManagementPolicy ?? PodManagementPolicy.ORDERED_READY);
  ...
  private _createHeadlessService() {
    ...
    if (ports.length === 0) {
      throw new Error(
        `Unable to create a service for the stateful set ${this.name}: StatefulSet ports cannot be determined.`,
      );
    }
    ...
    for (const port of ports) {
      ...
      if (!myPortNumbers.includes(targetPort)) {
        throw new Error(
          `Unable to expose stateful set ${this.name} via a service: Port ${targetPort} is not exposed by any container`,
        );
      }
    }
...
export class StatefulSetUpdateStrategy {
  ...
  public static rollingUpdate(
    options: StatefulSetUpdateStrategyRollingUpdateOptions = {},
  ...
  private constructor(
    private readonly strategy: k8s.StatefulSetUpdateStrategy,
  ) {}
