// yoinked from cdk8s-plus https://github.com/cdk8s-team/cdk8s-plus/blob/k8s-29/main/src/container.ts
...
export class EnvValue {
  ...
  public static fromConfigMap(
    ...
    key: string,
    options: EnvValueFromConfigMapOptions = {},
  ): EnvValue {
   ...
   * @param options: Additional options.
   */
  public static fromFieldRef(
    fieldPath: EnvFieldPaths,
    options: EnvValueFromFieldRefOptions = {},
  ): EnvValue {
    let needsKey: boolean = false;

    if (
      fieldPath === EnvFieldPaths.POD_LABEL ||
      fieldPath === EnvFieldPaths.POD_ANNOTATION
    ) {
      needsKey = true;
    }
  ...
  public static fromResource(
    ...
    options: EnvValueFromResourceOptions = {},
  ): EnvValue {
    const source: k8s.EnvVarSource = {
      resourceFieldRef: {
        resource: `${resource}`,
        ...(options.divisor
          ? { divisor: k8s.IntOrString.fromString(options.divisor) }
          : {}),
  ...
  public static fromProcess(
    key: string,
    options: EnvValueFromProcessOptions = {},
  ): EnvValue {
...
export class Container {
  ...
  constructor(props: ContainerProps) {
    if (props instanceof Container) {
      throw new Error(
        "Attempted to construct a container from a Container object.",
      );
    }
    ...
    if (props.port && props.portNumber) {
      throw new Error(
        "Either 'port' or 'portNumber' is allowed. Use 'portNumber' since 'port' is deprecated",
      );
    }
    ...
    this._startup =
      props.startup ??
      (this.portNumber ? defaultProbeConfiguration : undefined);
  ...
  public addPort(port: ContainerPort) {
    ...
    const numberProtocols = this._ports.map(
      (p) => `${p.number}/${p.protocol || Protocol.TCP}`,
    );
    ...
    if (numberProtocols.includes(protocol)) {
      throw new Error(
        `Port with number ${port.number} and protocol ${
  ...
  public _toKube(): k8s.Container {
    ...
    if (memoryLimit) {
      limits.memory = k8s.Quantity.fromString(
        memoryLimit.toMebibytes().toString() + "Mi",
      );
    }
    if (ephemeralStorageLimit) {
      limits["ephemeral-storage"] = k8s.Quantity.fromString(
        ephemeralStorageLimit.toGibibytes().toString() + "Gi",
      );
    }
    if (cpuRequest) {
    ...
    if (memoryRequest) {
      requests.memory = k8s.Quantity.fromString(
        memoryRequest.toMebibytes().toString() + "Mi",
      );
    }
    if (ephemeralStorageRequest) {
      requests["ephemeral-storage"] = k8s.Quantity.fromString(
        ephemeralStorageRequest.toGibibytes().toString() + "Gi",
      );
    }
...
export class Env {
  ...
  public constructor(
    sources: EnvFrom[],
    variables: { [name: string]: EnvValue },
  ) {
