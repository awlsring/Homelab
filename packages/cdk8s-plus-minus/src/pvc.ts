...
class ImportedPersistentVolumeClaim
  extends Construct
  ...
  implements IPersistentVolumeClaim
{
  ...
  public bind(vol: pv.IPersistentVolume) {
    if (this._volume && this._volume.name !== vol.name) {
      throw new Error(
        `Cannot bind claim '${this.name}' to volume '${vol.name}' since it is already bound to volume '${this._volume.name}'`,
      );
    }
    this._volume = vol;
  }
